...
clear
echo "SlackVPN Installer"
echo "------------------"
echo "1) OpenVPN"
echo "2) WireGuard"
echo "3) Exit"
echo
read -p "Select VPN type: " vpn_type
...
new_wg_client() {
    ...
    CLIENT_IP6="fddd:2c4:2c4:2c4::$octet"

    CLIENT_PRIV=$(wg genkey)
    CLIENT_PUB=$(echo "$CLIENT_PRIV" | wg pubkey)
    PSK=$(wg genpsk)

    # Add to config (like OpenVPN CCD logic)
...
[Peer]
PublicKey = $(grep PrivateKey /etc/wireguard/wg0.conf | awk '{print $3}' | wg pubkey)
PresharedKey = $PSK
...
if [[ ! -e /etc/wireguard/wg0.conf ]]; then
    ...
    clear
    echo "WireGuard installer (Slackware)"

    ip=$(ip -4 addr | grep inet | grep -v 127 | awk '{print $2}' | cut -d/ -f1 | head -1)
    ip6=$(ip -6 addr | grep 'inet6 [23]' | awk '{print $2}' | cut -d/ -f1 | head -1)

    read -p "Public IPv4 [$ip]: " public_ip
    ...
    [[ -n "$pkgs" ]] && slackpkg install $pkgs

    mkdir -p /etc/wireguard /etc/iptables
...
EOF
    ...
    grep -q ip_forward $rclocal || echo "echo 1 > /proc/sys/net/ipv4/ip_forward" >> $rclocal
    grep -q ipv6/conf/all/forwarding $rclocal || echo "echo 1 > /proc/sys/net/ipv6/conf/all/forwarding" >> $rclocal

    # IPTABLES (same style as your OpenVPN)
    ...
    if [[ -n "$ip6" ]]; then
        ip6tables -t nat -A POSTROUTING -s fddd:2c4:2c4:2c4::/64 ! -d fddd:2c4:2c4:2c4::/64 -j SNAT --to $ip6
        ip6tables -I FORWARD -s fddd:2c4:2c4:2c4::/64 -j ACCEPT
        ip6tables -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
    fi

    iptables-save > /etc/iptables/rules.v4
    [[ -n "$ip6" ]] && ip6tables-save > /etc/iptables/rules.v6

    grep -q "iptables-restore" $rclocal || echo "iptables-restore < /etc/iptables/rules.v4" >> $rclocal
    grep -q "ip6tables-restore" $rclocal || echo "ip6tables-restore < /etc/iptables/rules.v6" >> $rclocal

    wg-quick up wg0
    grep -q "wg-quick up wg0" $rclocal || echo "wg-quick up wg0" >> $rclocal
    ...
    case "$opt" in
        ...
        ;;
        2)
            clients=$(grep '^# BEGIN_PEER' /etc/wireguard/wg0.conf | awk '{print $3}')
            if [[ -z "$clients" ]]; then
                echo "No clients."
                exit
            fi
            ...
            client=$(echo "$clients" | sed -n "${num}p")

            read -p "Confirm removal of $client? [y/N]: " confirm
            [[ ! "$confirm" =~ ^[yY]$ ]] && exit

            PUB=$(sed -n "/BEGIN_PEER $client/,/END_PEER $client/p" /etc/wireguard/wg0.conf | grep PublicKey | awk '{print $3}')
        ...
        3)
            ...
            sed -i '/wg-quick up wg0/d' $rclocal
